...
1.增加设置昵称功能。
2.增加好友验证和群邀请验证设置功能。
3.修复未登录状态不能进入账号切换选择界面选择已登录过账号的问题。
4.增加设置群名片功能。
5.uniapp增加别名显示，显示顺序为(群组内)群名片>用户昵称>用户名>用户id。
6.uniapp增加群聊会话界面提示@功能。
7.修复自动登录时不能正确解析link及code参数的问题。
8.merge 最新floo web sdk修复多个sdkbug。
9.修改uni app css文件加载方式，解决andoid app 打包出现错误的问题。
10.修复群成员列表展示时没有默认头像没有出现的bug。
11.修改新建群组逻辑，新建群组时添加成员列表优先展示50个最近单聊会话联系人信息，然后在拼接上好友信息。

### v3.0.7 20240222
1.更新@列表多选图标，默认没有选中时为灰色，有选中成员时设置为蓝色。
