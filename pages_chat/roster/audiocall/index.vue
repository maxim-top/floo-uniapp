  ...
  <view class="container" style="height: 100%">
    ...
    <view class="header_view">
      ...
      <view class="nick">
        ...
        <text v-if="!caller" class="nick_tips" :style="{ display: isGetThrough ? 'none' : 'block' }">收到一个语音通话请求...</text>
      </view>
    </view>
    <view class="buttons">
      ...
      <view class="button_item" @tap="speakerChangeStatus">
        <image :src="speakerPic" class="button_avatar"></image>
        <text v-if="speaker">扬声器打开</text>
        <text v-if="!speaker">扬声器关闭</text>
      </view>
    </view>
    ...
    <view>
      <audio ref="vAudio" class="hide" autoplay playsinline muted />
    </view>
  </view>
</template>
...
export default {
  data() {
    return {
      ...
      speakerPic: '/static/pages/image/call/speaker_on.png',
  ...
  mounted: function () {
    ...
    this.$refs.vAudio.$refs.audio.muted = false;
  },
  ...
  methods: {
    ...
    onRosterRTCMessage(message) {
      ...
      if (!isHistory && config && !isNative && (fromUid == this.uid || (fromUid == uid && toUid == this.uid))) {
        if (config.action && config.action === 'call' && config.initiator) {
          ...
          if (fromUid === uid) {
            // current user other device hangup call. just ignore and navigateBack.
          } else {
            if (callStatus) {
    ...
    speakerChangeStatus() {
      ...
      this.setData({
        speaker,
        speakerPic: speaker ? '/static/pages/image/call/speaker_on.png' : '/static/pages/image/call/speaker_off.png'
      });
    },
    ...
    attachStream(stream, type, isLocal = false) {
      if (type === 'audio') {
        this.$refs.vAudio.$refs.audio.srcObject = stream;
        this.$refs.vAudio.$refs.audio.play();
      }
    },
